...
public static class GMath
{
    ...
    // find a scale that you can apply to an image that will fit it inside a space without distortion
    public static float FindScaleToFitImage(Texture2D tex, int w, int h)
    {
        return FindScaleToFit(tex.Width, tex.Height, w, h);
    }
    //scale scale size to fit in w/h
    public static float FindScaleToFit(int scaleW, int scaleH, int w, int h)
    {
        float sy = 1f * h / scaleH;
        float sx = 1f * w / scaleW;
        return Math.Min(sx, sy);
    }
    public static float FindScaleToFit(Vector2 a, Vector2 b, int w, int h)
    {
        return FindScaleToFit((int)Math.Abs(a.X - b.X), (int)Math.Abs(a.Y - b.Y), w, h);
    }
    public static float FindScaleToFillWithImage(Texture2D tex, int w, int h)
        => FindScaleToFill(tex.Width, tex.Height, w, h);

    public static float FindScaleToFill(int scaleW, int scaleH, int w, int h)
    {
        float sy = 1f * h / scaleH;
        float sx = 1f * w / scaleW;
        return Math.Max(sx, sy);
    }
    public static float FindScale(int sourceDim, int targetDim)
        => 1f * targetDim / sourceDim;
    public static float FindScale(float sourceDim, float targetDim)
        => 1f * targetDim / sourceDim;

    public static int FindWidthForHeightProportional(int baseW, int baseH, int H)
    {
        return baseW * H / baseH;
    }
    public static int FindHeightForWidthProportional(int baseW, int baseH, int W)
    {
